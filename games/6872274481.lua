...
run(function()
    ...
    repeat
        ...
        -- Add a condition to check if the player is alive before attacking
        if plr and (not Legit.Enabled or (lplr.Character:GetAttribute('Health') or 0) > 0) and (lplr.Character:GetAttribute('Health') or 0) > 0 then
            local localPosition = entitylib.character.RootPart.Position
    ...
    -- Add a condition to stop the repeat when the player is dead
    until (lplr.Character:GetAttribute('Health') or 0) <= 0
end
