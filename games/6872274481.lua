...
local cloneref = cloneref or function(obj)f
	return obj
end
...
run(function()
    ...
    AimAssist = vape.Categories.Combat:CreateModule({
            ...
            if callback then
                ...
                AimAssist:Clean(runService.Heartbeat:Connect(function(dt)
                    if entitylib.isAlive and store.hand.toolType == 'sword' and ((not ClickAim.Enabled) or (tick() - bedwars.SwordController.lastSwing) < 0.4) then
                        ...
                        if currentTarget and currentTarget.Parent and currentTarget:FindFirstChild('RootPart') then
                            local delta = (currentTarget.RootPart.Position - entitylib.character.RootPart.Position)
                            local localfacing = entitylib.character.RootPart.CFrame.LookVector * Vector3.new(1, 0, 1)
                            local angle = math.acos(localfacing:Dot((delta * Vector3.new(1, 0, 1)).Unit))
                            if angle <= (math.rad(AngleSlider.Value) / 2) and delta.Magnitude <= Distance.Value then
                                -- Current target is still valid, keep aiming at it
                                gameCamera.CFrame = gameCamera.CFrame:Lerp(CFrame.lookAt(gameCamera.CFrame.p, currentTarget.RootPart.Position), (AimSpeed.Value + (StrafeIncrease.Enabled and (inputService:IsKeyDown(Enum.KeyCode.A) or inputService:IsKeyDown(Enum.KeyCode.D)) and 10 or 0)) * dt)
                                return
                            else
                                -- Current target is no longer valid, reset it
                                currentTarget = nil
                            end
                        end

                        -- If no current target or it's no longer valid, find a new one
                        if ent then
                            local delta = (ent.RootPart.Position - entitylib.character.RootPart.Position)
                            local localfacing = entitylib.character.RootPart.CFrame.LookVector * Vector3.new(1, 0, 1)
                            local angle = math.acos(localfacing:Dot((delta * Vector3.new(1, 0, 1)).Unit))
                            if angle <= (math.rad(AngleSlider.Value) / 2) then
                                currentTarget = ent  -- Set the new target
        ...
        Tooltip = 'Smoothly aims to closest valid target with sword. In Single mode, it locks onto the target and does not switch unless the target is no longer valid.'
    })
				...
				repeat
					if entitylib.isAlive then
						...
						if wool then
							...
							-- Calculate the position directly below the player
							...
							if not block then
								-- Place a block directly below the player
								task.spawn(bedwars.placeBlock, currentpos, wool, false)
	
								-- Place a block one block below if possible
								local belowPos = currentpos - Vector3.new(0, 3, 0)
								if not getPlacedBlock(belowPos) then
					...
					task.wait() -- Remove or minimize delays for maximum speed
