			...
			return {
				...
				SendToServer = function(_, attackTable, ...)
					...
					if Reach.Enabled or HitBoxes.Enabled then
						...
						attackTable.validate.selfPosition.value += CFrame.lookAt(selfpos, targetpos).LookVector * math.max((selfpos - targetpos).Magnitude - 0, 0)
					end
							...
							for _, v in plrs do
								...
								if not Attacking then
									...
									if not Swing.Enabled and AnimDelay <= tick() and not LegitAura.Enabled then
										AnimDelay = tick() + (meta.sword.respectAttackSpeedForEffects and meta.sword.attackSpeed or (Sync.Enabled and 0 or 0))
